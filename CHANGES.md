...
- Moved the QHTML Qt/WASM runtime assets into `dist/qhtml-wasm/`.
- Updated q-property reference storage so QHTML named-instance assignments are stored internally by UUID and resolve through the live UUID lookup on public reads.
...
- Added `q-parallel-animation` and `q-sequential-animation` component aliases for the animation group components.
- Added `q-mouse-area` with overlapping-area mouse hit detection, entry/exit/button signals, and local/global `QPoint` coordinate helpers.
- Added `QPoint(x, y)` as a coordinate model equivalent to `QMap({ x, y })`.
