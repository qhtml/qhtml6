...
- QDom color helpers:
  - `qcolor(name)` returns a `QColorNode` for a schema area or theme name from cascading color metadata (`qdom.meta.qColorSchemas` / `qdom.meta.qColorSchemaDefs` / `qdom.meta.qColorThemes`)
  - schema metadata entries are area->CSS-property mappings used by runtime `q-color` application
  - `qcolors()` returns all discovered color nodes (schemas + themes)
